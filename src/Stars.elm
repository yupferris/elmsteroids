...
init =
    step (int 80 100) >>= init_


init_ : Int -> State Seed (List Star)
init_ count =
    if count == 0 then
        return []
    else
        (::) <$> initStar <*> init_ (count - 1)
...
drawStar star =
    let
        blink =
            sin (star.blinkPhase) * 0.4 + 0.6
    in
        rect 1 1 |> filled white |> move star.position |> alpha blink
